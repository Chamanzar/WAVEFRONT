...
for ss = 1:Sessions_size
    ...
    % In the next block of code CSD events are numbered
    ...
    for i=1:size(EEG.event,2) % Loop through all events
        % find all 'Spreading Depolarization' events
        if(strcmp(EEG.event(i).type,'CSD') || strcmp(EEG.event(i).type,'CSD/ISD') ||...
            ...
            % Store SD Labels for tracking/'visualizing' SD events 
            CSD_Labels = cat(1,CSD_Labels,EEG.event(i));
            % Update numerical prefix for each new SD
            CSD_ind = CSD_ind+1;
        end
    end
    % Mask any region of the data where Impedance is negative 
    ...
    % Add ECoG channels to EEG structure:
    for ECoG_ind=1:6
        EEG.data(end+1,:) = ECoG.data(ECoG_ind,:);
    end

    % Update 'number of chans' to account for newly added ECoG channels
    EEG.nbchan = size(EEG.data,1);

    % Add appropriate label at each new ECoG Channel
    ...
    eeglab redraw % Refresh eeglab graphical interface
    pop_eegplot( EEG, 1, 1, 1); % Replot data to visualize EEG + ECoG data
    
    % Save Session-wide 'EEG' struct which contains EEG, ECoG & Event info
